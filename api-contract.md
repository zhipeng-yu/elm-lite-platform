...
- 加入及修改数量时商品必须存在、上架，且所有用户购物车中的该商品数量合计不得超过实时库存；不存在返回 404，下架或库存不足返回 409。公开商品列表与详情的 `stock` 返回实时库存减去所有购物车占用后的可用量，最低为 0；商家商品接口仍返回数据库实际库存。下单再次执行最终库存校验。
...
- 购物车加减只修改购物车数量，不直接修改数据库库存；购物车数量作为临时占用参与顾客可用库存展示和加购校验。库存只在成功创建订单时实际扣减，沿用现有扣减和回滚逻辑。
...
管理员订单列表与详情均返回 `productAmountCent`、`discountAmountCent`、`deliveryFeeCent`、`totalAmountCent`，按“商品 − 优惠 + 配送”核对快照；无优惠的旧订单返回优惠 0，不能用现时券面额重算历史订单。
