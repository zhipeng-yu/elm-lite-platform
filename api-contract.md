...
- 梁实现核销/返券方法，余负责订单生命周期；对接方法参加调用方事务，统一锁顺序，禁止另开独立事务。具体方法签名在开始任务时共同确认。
...
商家查询采用以下字段约定；用券方法签名与锁顺序仍为协作提案，须由余、梁共同确认后采用，不改变第 9.1、9.2、9.4 节已约定的业务范围。
...
建议由梁在优惠券 Service 提供以下内部方法，余在订单生命周期中调用；这里仅约定提案，不提前创建空实现或 HTTP 接口：

| 建议方法 | 调用点及行为 |
| --- | --- |
| `long consume(long userId, long shopId, long userCouponId, long orderId, long productAmountCent)` | 在订单事务已取得订单 ID、最终提交前核验并核销用户券，返回实际优惠整数分；余保存券关联及优惠快照，再更新应付金额。未选券不调用，保持原有流程 |
| `void restore(long userId, long orderId)` | 首次取消并恢复库存时，按订单的券关联返还；未用券或已返还无额外变更，不延长有效期 |

两个方法均使用 `MANDATORY`，不得另开事务；任一步失败必须回滚订单、库存、购物车和券。核销必须核验第 9.4 节全部条件，关联错误不得默默当作未用券。

锁顺序提案：创建沿用“用户→所选购物车→商品 ID 升序→店铺→新订单”，再核销券；取消使用“用户→已有订单→商品 ID 升序→用户券”；商家确认及骑手状态动作只锁目标订单并作条件更新。梁须共同核对券模板启停、领取和核销的锁顺序，不能在持有券锁后反向请求用户、商品或订单锁。并发核销、取消与确认竞争及失败回滚仍须在对应开发日用测试验证。
