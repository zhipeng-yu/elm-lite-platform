...
13. 一个商品可以拥有多张有序详情图（接口最多 3 张）；删除商品时级联删除详情图。
...
15. `user_coupon.status` 仅保存未使用/已使用；过期、停用和是否满足门槛由券模板及当前时间计算。重复取消旧订单不能再次返券或恢复库存。
16. 删除用户券时订单关联置空，优惠金额快照不变；当前无删除用户券的公开接口。
