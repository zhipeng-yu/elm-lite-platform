...
- D5 三个 PR 均已合并；真实 MySQL 并发/外键复验、关闭 mock 的注册至订单查询浏览器联调、人工交叉验收尚未执行，分别由梁、龙及小组继续完成。H2 与 mock 测试通过不等同于最终验收。
- D6 的具体需求变更待老师公布并由小组确认，暂不预设需求或开工日期；当前先完成 D5 遗留联调。梁后续复核未被生产调用的库存/清理辅助方法是否需要保留，订单继续统一使用 `CheckoutService`。
