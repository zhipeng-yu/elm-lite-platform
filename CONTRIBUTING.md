...
| 龙（B）：数据库与后端业务 | 数据库迁移、商家、店铺、分类、商品、购物车 |
| 梁（C）：前端与验收 | Vue、Router、Axios、公共组件、联调和验收材料 |
...
```text
同步 main -> 创建功能分支 -> 用 VS Code 写代码 -> 提交 -> push -> 网页创建 PR -> 队友审核 -> 合并
```
...
写完测试后，先运行项目约定的测试命令，确认新增测试会因为功能尚未实现而失败，并保留结果。具体测试命令在项目脚手架确定后补充到本 README。
