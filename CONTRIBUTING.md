...
| 角色 | 主要职责 |
| --- | --- |
| 余（A）：订单履约 | 商家订单、状态流转、顾客取消、库存恢复、骑手端及公共鉴权 |
| 梁（B）：商品营销 | 商品多图与介绍、商家券管理、顾客领券与用券、优惠快照及返券 |
| 龙（C）：移动端与管理员 | 顾客手机适配、管理员登录/查询/账号启停、前端公共布局及最终风格统一 |

各人负责板块所需的数据库、接口、页面、自测和 PR；每个后端接口和核心业务方法仍按小批红—绿开发。余维护测试规范，各人审阅并解释自己的测试和实现。
...
| 数据库迁移编号、初始化 SQL 和数据库说明 | B 协调编号；各板块负责人编写自己的迁移和测试数据 |
...
商品详情功能由梁完成，龙在交付后统一样式；取消订单和骑手归属由余维护。梁接入优惠券前先同步已合并的订单改动，核销/返券参加同一订单事务。结算和订单页面按任务先后交接，避免多人同时编辑。新增路由由龙接入，新增身份的公共授权由余接入；管理员账号通过初始化创建，不开放管理员自助注册。

迁移编号由梁在对应 Issue 分配，每个独立数据库变更使用新编号；已经合并或应用的迁移不再修改，不将不同日期交付的功能反复追加到同一个迁移文件。

## 2. 板块交付与每日看板

- 开始前约定接口、状态规则和共享文件交接；各人独立开发、自测，完成一项可运行的小任务即提交 PR，满足测试与审核条件后合并。
- 中途进行一次真实接口联调，最后统一完整回归和视觉；各人修复自己的板块问题。
...
- `handoff.md` 只保留每日任务表。状态、PR 链接、简短测试结果和阻塞写进对应任务格；详细测试证据、AI 使用目的、涉及文件和人工复核人写 PR 描述，不另建流水文档。
